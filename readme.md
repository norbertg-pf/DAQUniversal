...
2. In the **Select Active Channels** popup, configure the Keithley DMM settings (IP, sample rate, timeout).
3. (Optional) In Channel Config, choose the DMM backend (`pyvisa` or `keithley_daq6510 (auto)`).
4. Open channel selection and enable the `DMM` signal.
