...
class DAQControlApp(QWidget):
    ...
    def _is_simulated_device(self, device_name):
        ...
        return dev.startswith("simulated") or "simulated" in dev or "simulated" in ptype
    ...
    def open_channel_selector(self):
        ...
        if dialog.exec_():
            ...
            if len(selected) == 0:
                ...
                kept_math = [s for s in self.available_signals if s.startswith("MATH")]
                kept_dmm = [s for s in self.available_signals if s == "DMM" and s not in allowed_signals]
                self.available_signals = selected + kept_dmm + kept_math
