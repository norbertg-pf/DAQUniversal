...
class DAQControlApp(QWidget):
    ...
    def _is_simulated_device(self, device_name):
        name = (device_name or "").lower()
        ptype = (self.device_product_types.get(device_name, "") or "").lower()
        return name.startswith("simulated") or "simulated" in name or "simulated" in ptype

    def _allowed_signals_for_current_device(self):
        if self._is_math_device_selected():
            return [f"MATH{i}" for i in range(4)]
        if self._is_dmm_device_selected():
            return ["DMM"]

        selected_device = self.device_cb.currentData()
        allowed = []
        if selected_device and not self._is_simulated_device(selected_device):
            allowed.extend(self._channels_for_device(selected_device))
        allowed.append("DMM")
        return allowed
...
# --- PYQTGRAPH REPLACEMENT FOR MATPLOTLIB ---
    ...
    def apply_selected_device_profile(self):
        allowed = set(self._allowed_signals_for_current_device())
        kept_math = [s for s in self.available_signals if s.startswith("MATH")]
        if self._is_math_device_selected():
            self.available_signals = [s for s in self.available_signals if s in allowed]
        else:
            kept_hw = [s for s in self.available_signals if s in allowed]
    ...
    def open_channel_selector(self):
        ...
        allowed_signals = self._allowed_signals_for_current_device()
        if self._is_math_device_selected():
